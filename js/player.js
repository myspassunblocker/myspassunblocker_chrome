...
function initializeFSKVideo(videoid, chapter, divid, startposition) {  
	...
	_initializePlayer(videoid,divid,startposition,videoAppBasePath+"/myspass_fsk/", chapter);
}
...
function initializeNoAdVideo(videoid,chapter,divid, startposition){
    ...
    var so = new SWFObject("/myspass/includes/apps/player/standard/werbefrei_workaround/player_core.swf", "player", "100%", "100%", "9","#000000");
        so.useExpressInstall("/"+project+"/media/flash/player/expressinstall.swf");
	so.addParam("menu","false");
	so.addParam("wmode","opaque");
	so.addParam("quality","high");
	...
	//	alert(i+"=="+vp_global[videoid][i])
        ...
        so.addVariable("config", "/myspass/includes/apps/player/standard/config_myspass.xml");
...
function initializeNoAdNoAutoplayVideo(videoid,chapter,divid, startposition){
	...
	so.addVariable("config", "/myspass/includes/apps/player/standard/config_noatoplay_myspass.xml");	
